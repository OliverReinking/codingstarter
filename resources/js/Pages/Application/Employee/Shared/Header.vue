    ...
    <div class="flex items-center space-x-2 md:flex-nowrap text-sm font-medium">
      ...
      <div>
        <dropdown-link
          :with-icon="false"
          :with-route="true"
          :route-name="route('employee.chat.inbox.index', 'unread')"
        >
          <badge-unread-chats :count="$page.props.userdata.unreadCompanyChats" />
        </dropdown-link>
      </div>
      <div
      ...
      <div>
        <dropdown align="right" width="96">
          ...
          <template #content>
            <!-- Account Management -->
            <div class="block px-4 py-2 text-xs text-layout-400">Meine Daten</div>

            <dropdown-link :route-name="route('employee.profile')" :with-route="true">
              Persönliche Einstellungen
            </dropdown-link>

             <dropdown-link
              :route-name="route('employee.api_tokens.index')"
              :with-route="true"
